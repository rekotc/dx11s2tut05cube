...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight, 
						   XMMATRIX baseViewMatrix, int x, int y)
{
	bool result;
...
bool TextClass::SetMessage(string message, int ypos, ID3D11DeviceContext* deviceContext){
	...
	//strcat_s(baseSentence, message);
	
	//sprintf_s(tempString, "%f", distance);
	//strcat_s(string, tempString);
	baseSentence += message;
	result = UpdateSentence(m_sentence1, baseSentence, 20, ypos, 0.0f, 0.0f, 0.0f, deviceContext);
