...
bool LightShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, 
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, XMFLOAT4 pickedUpColor)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightDirection, pickedUpColor);
	if(!result)
	{
		return false;
	}
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, XMFLOAT4 pickedUpColor)
{
	...
	// Copy the lighting variables into the constant buffer.
	//dataPtr3->colorPickedUp = D3DXVECTOR4(0.2f,0.5f,0.7f,1.0f);
	dataPtr3->colorPickedUp = pickedUpColor;
